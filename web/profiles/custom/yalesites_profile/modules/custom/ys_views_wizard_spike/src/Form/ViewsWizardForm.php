 ...
 * page) is on the spike/views-wizard-redirect branch; everything except the
...
class ViewsWizardForm extends FormBase {
  ...
  public function openConfigureForm(array &$form, FormStateInterface $form_state): AjaxResponse {
    ...
    // Same seeding problem as prototype A, same solution - but here the query
    // parameter has to be put on the request by hand, because we are building
    // the form in-process rather than redirecting to a URL that carries it.
    if ($this->options->needsSeed($content_type, $view_mode)) {
      $this->getRequest()->query->set('ys_wizard_seed', $content_type . ':' . $view_mode);
    }

    $configure_form = $this->formBuilder->getForm(AddBlockForm::class, $section_storage, $delta, $region, $plugin_id);

    // Load bearing, and only half a fix. FormBuilder defaults #action to the
    // current request URI, which here is the WIZARD's route - without this the
    // embedded form would POST back to a route that builds a different form.
    // It does NOT fix the embedded form's own #ajax URLs: those are derived
    // from the request too, so every element of AddBlockForm still calls back
    // to /ys-views-wizard/choose/... and fails with Drupal.AjaxError. See the
    // README - this is the finding that sinks approach B.
