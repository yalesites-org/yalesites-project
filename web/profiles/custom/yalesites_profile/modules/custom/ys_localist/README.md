...
## Location fields: Localist place vs. free-form location

Events that are not synced from Localist (or that don't have a Localist place configured) have no way to describe where they take place using the `field_event_place`/`field_event_room` fields, since those are populated by the `localist_places` migration and are only meaningful once Localist is configured. To support non-Localist events, the Event content type also has a `field_location_additional_info` WYSIWYG field ("Additional Information") for free-form location details: addresses, building names, virtual platform links, hybrid instructions, etc.

- **Editing:** `field_event_place` ("Yale Location") and `field_event_room` ("Room") live in the "Localist data" tab of the event edit form, alongside the other Localist-synced fields, since they are only relevant when Localist is in use. `field_location_additional_info` lives in the "Basic info" tab so it's available for every event.
- **Display precedence:** On the event page, if a Localist place (`field_event_place`) is set, it is always shown and `field_location_additional_info` is suppressed, even if both fields have a value. If no Localist place is set, `field_location_additional_info` is shown instead. This logic lives in the `{% if place %}...{% elseif location_additional_info %}...{% endif %}` block of `yds-event-meta-localist.twig` in the [component-library-twig](https://github.com/yalesites-org/component-library-twig) repo (see `components/02-molecules/meta/event-meta/yds-event-meta-localist.twig`).
- **Data flow:** `Drupal\ys_localist\MetaFieldsManager::getEventData()` reads `field_location_additional_info` (via `check_markup()`, matching the pattern used for other rich-text fields such as `field_citation`/`field_abstract` in `ResourceMetaBlock`) and returns it as `location_additional_info`. `EventMetaBlock::build()` passes it through as `#location_additional_info`, and it's declared as a theme variable in `ys_layouts_theme()` alongside `place` and `room`.
- **Accessibility:** the Localist place markup includes `schema.org/Place` and `schema.org/PostalAddress` microdata (`itemprop`/`itemscope`/`itemtype`); the free-form fallback is marked with `itemprop="location"` (schema.org allows `Event.location` to be `Place` or plain `Text`). Both branches keep the same "Location" label immediately before the value so screen readers announce it in the same logical order regardless of which source is used.
