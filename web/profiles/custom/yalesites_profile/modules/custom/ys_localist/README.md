...
## Location fields: Yale Location/Room vs. free-form location

The Event content type has three location-related fields, grouped together in a "Location" fieldset in the "Basic info" tab of the edit form: `field_event_place` ("Yale Location", an `event_place` taxonomy reference with address/geo data), `field_event_room` (a free-text room string), and `field_location_additional_info` (a WYSIWYG "Additional Information" field for free-form details: addresses, building names, virtual platform links, hybrid instructions, etc.). All three are editable by any event author — Place and Room aren't exclusive to Localist-synced events; any event can have them set manually.

- **Editing:** all three fields live together in the "Location" fieldset in "Basic info". They are independent of each other — an event can have any combination set (e.g. Room without Place, or Additional Info alongside a Localist-synced Place). `field_localist_event_experience` ("Event Experience") is the only field in this area that's locked/disabled, since it's synced/overwritten by the Localist migration (see `overwrite_properties` in `migrations/localist_events.yml`) — it lives in the "Localist data" tab, not the Location fieldset.
- **Display logic:** on the event page, Place and/or Room render together as one block (gated on `place or room` — either can appear alone) with `schema.org/Place`/`PostalAddress` microdata. `field_location_additional_info` **always** renders when it has a value, regardless of whether Place/Room are also shown — it is not a fallback. When it renders alongside Place/Room, it's labeled "Additional Information" (avoiding a duplicate "Location" label) and doesn't carry `itemprop="location"` (Place/Room already claims that structured-data property); when it renders alone, it's labeled "Location" and carries `itemprop="location"`. If none of the three fields have a value, nothing renders. This logic lives in `yds-event-meta-localist.twig` in the [component-library-twig](https://github.com/yalesites-org/component-library-twig) repo (see `components/02-molecules/meta/event-meta/yds-event-meta-localist.twig`).
- **Data flow:** `Drupal\ys_localist\MetaFieldsManager::getEventData()` reads `field_location_additional_info` as a `#type: processed_text` render array (so cacheability/attachments from filters like MathJax/Linkit on the `basic_html` format bubble correctly — see `ResourceMetaBlock` for the equivalent pattern with `field_citation`/`field_abstract`) and returns it as `location_additional_info`. `EventMetaBlock::build()` passes it through as `#location_additional_info`, and it's declared as a theme variable in `ys_layouts_theme()` alongside `place` and `room`.
- **Accessibility:** both the Place/Room block and the free-form field keep the "Location" (or "Additional Information") label immediately before the value, in DOM order, so screen readers announce it in a logical order regardless of which fields are populated.
