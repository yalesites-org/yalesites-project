...
## Location fields: Localist location/room vs. native Address/Additional Address Information

The Event content type has two pairs of location-related fields, split by who owns the data:

- **Localist-owned** (in the "Localist data" tab, disabled/locked like the other synced fields — see `_ys_core_disable_event_fields()` in `ys_core.module`): `field_event_place` ("Localist location", an `event_place` taxonomy reference with address/geo data) and `field_event_room` ("Localist room", a free-text string). Both are synced/overwritten by the Localist migration (see `overwrite_properties` in `migrations/localist_events.yml`), so they're locked to prevent edits that would be silently wiped on the next sync.
- **Native/Drupal-owned** (in the "Location" fieldset in the "Basic info" tab, always editable): `field_event_address` (an `address`-module field, same settings as the `field_address` field on the `event_place` taxonomy term) and `field_address_additional_info` (a WYSIWYG "Additional Address Information" field for free-form details: virtual platform links, hybrid instructions, building notes, etc. — pairs with the structured Address field).

**Display logic:** the structured location branches on whether the event is Localist-synced at all, not on which individual fields have values. `Drupal\ys_localist\MetaFieldsManager::getEventData()` computes `is_localist_event` from whether `field_localist_id` is populated (that field is set only by the `localist_events` migration, never by manual entry). If `is_localist_event` is true, the Localist location/room block renders (with `schema.org/Place`/`PostalAddress` microdata and a map) and the native Address is ignored even if it has a value. If `is_localist_event` is false, the native Address renders (with `schema.org/PostalAddress` microdata, no map) and the Localist fields are ignored. Additional Address Information is not gated: it renders on every event where it is populated, Localist or not. It is labelled "Additional Address Information" when a place, room or native Address rendered above it, and otherwise stands in as the "Location" (carrying `itemprop="location"`). It's possible for nothing to render (e.g. a Localist event with no synced location and no additional information). This logic lives in `yds-event-meta-localist.twig` in the [component-library-twig](https://github.com/yalesites-org/component-library-twig) repo (see `components/02-molecules/meta/event-meta/yds-event-meta-localist.twig`).
- **Hand-authored events that used place/room:** `ys_localist_deploy_10001()` (in `ys_localist.deploy.php`) copies the place term's address into `field_event_address` and the place name and room into `field_address_additional_info` on events with no `field_localist_id`, filling only empty fields, since those events would otherwise lose their location once place/room became Localist-only.

- **Data flow:** `MetaFieldsManager::getEventData()` returns `is_localist_event`, `place_info`, `room`, `event_address`, and `address_additional_info` (the last as a `#type: processed_text` render array, so cacheability/attachments from filters like MathJax/Linkit on the `basic_html` format bubble correctly — see `ResourceMetaBlock` for the equivalent pattern with `field_citation`/`field_abstract`). `EventMetaBlock::build()` passes these through as `#is_localist_event`/`#place`/`#room`/`#event_address`/`#address_additional_info`, declared as theme variables in `ys_layouts_theme()`.
- **Accessibility:** whichever block renders keeps its "Location" (or "Additional Address Information", when a place, room or native Address rendered above it) label immediately before the value, in DOM order, so screen readers announce it in a logical order.
...
This module has PHPUnit tests under `tests/src/`. They mock all HTTP and never contact the live Localist API. Run them from the project root on the local Lando environment, passing the module's `tests` path so PHPUnit only discovers this module's tests (not Drupal core/contrib):

```bash
lando ssh -c "env SIMPLETEST_DB=mysql://pantheon:pantheon@database/pantheon \
  php /app/vendor/bin/phpunit -c /app/phpunit.xml \
  /app/web/profiles/custom/yalesites_profile/modules/custom/ys_localist/tests"
```
