...
**Display logic:** the event page branches on whether the event is Localist-synced at all, not on which individual fields have values. `Drupal\ys_localist\MetaFieldsManager::getEventData()` computes `is_localist_event` from whether `field_localist_id` is populated (that field is set only by the `localist_events` migration, never by manual entry). If `is_localist_event` is true, the Localist location/room block renders (with `schema.org/Place`/`PostalAddress` microdata and a map) — the native Address/Additional Address Information fields are ignored even if they happen to have values. If `is_localist_event` is false, the native Address (with `schema.org/PostalAddress` microdata, no map) and Additional Address Information fields render instead — the Localist fields are ignored. It's possible for nothing to render in either branch (e.g. a Localist event with no synced location, or a native event with neither field filled in). This logic lives in `yds-event-meta-localist.twig` in the [component-library-twig](https://github.com/yalesites-org/component-library-twig) repo (see `components/02-molecules/meta/event-meta/yds-event-meta-localist.twig`).
...
- **Accessibility:** whichever block renders keeps its "Location" (or "Additional Address Information", when paired with a native Address) label immediately before the value, in DOM order, so screen readers announce it in a logical order.
