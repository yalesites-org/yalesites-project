 ...
 */
class UpdateProfileMetaBlockCommands extends DrushCommands {
  ...
  public function updateProfileMetaBlocks(): void {
    ...
    foreach ($nids as $nid) {
      ...
      try {
        $node->save();
        $this->output()->writeln("Updated profile meta block for node $nid");
      }
      catch (\Exception $ex) {
        $this->output()->writeln("Failed to update profile meta block for node $nid");
      }
    }
