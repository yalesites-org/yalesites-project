...
| `Unit/CardSizeTest` | The shared "Card size" dial (#1648): offered on every content type's card grid with large/small options defaulting to large, absent from list/condensed/directory, the declarative `supports_card_size` capability, and `normalizeCardSize()` falling back to large for any value outside the offered set. |
