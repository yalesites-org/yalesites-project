...
block whose form branched on content type and display mode, there are now **13
...
`post_card`, `profile_condensed`). The display mode is **not** a form control —
...
unconverted instance.

The former `profile_directory` bundle is retired (#1682): the
`ys_views_basic_post_update_retire_profile_directory()` post-update converts
