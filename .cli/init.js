 ...
 * replaces all instances of `yalesites` with the given machineName.
 *
 * @param {string} machineName string that should replace yalesites.
 * @returns {function} fn that when called with a str, replaces all instances of `yalesites` with the given machineName.
 */
const strReplaceEmulsify = (machineName) => (str) =>
  str.replace(/yalesites/g, machineName);
...
const main = () => {
  ...
  renameFiles([
    {
      from: '../yalesites.info.yml',
      to: `../${machineName}.info.yml`,
    },
    {
      from: '../yalesites.theme',
      to: `../${machineName}.theme`,
    },
    {
      from: '../yalesites.libraries.yml',
